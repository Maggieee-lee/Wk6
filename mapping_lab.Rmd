...
```{r}
# Load unintentional injuries data
unintentional_injuries <- read.csv('https://www.opendata.nhs.scot/dataset/b0135993-3d8a-4f3b-afcf-e01f4d52137c/resource/aee43295-2a13-48f6-bf05-92769ca7c6cf/download/ui_admissions_2023.csv')
```
```
...
```{r}
injuries <- unintentional_injuries %>% 
  filter(Sex == "Female", AgeGroup == "75plus years",InjuryType == "Falls", FinancialYear == 
           "2022/23", HBR != "S92000003", InjuryLocation == "Home") 

falls_admission_75_summary <- injuries %>% 
  group_by(HBR) %>% 
  summarise(Totaladmission = sum(NumberOfAdmissions))
read_sf("data/NHS_healthboards_2019.shp")
NHS_healthboards <- st_read("data/NHS_healthboards_2019.shp")

falls_admissions_hb <- NHS_healthboards %>% 
  full_join(falls_admission_75_summary, join_by(HBCode == HBR))
```
...
```{r}
seniorfemalefallgraphs <- falls_admissions_hb %>% 
  ggplot(aes(fill = Totaladmission)) +
  geom_sf(colour = "black")
  # scale_fill_distiller
  

seniorfemalefallgraphs
```
